 ...
 * @return $text
 */
function prism_trim_excerpt($text) {
	if ( '' == $text ) {
...
/**
 * prism_the_excerpt function.
 * 
 * @param string $deprecated (default: '')
 * @return $output
 */
function prism_the_excerpt( $deprecated = '' ) {
	global $post;
	$output = '';
	$output = strip_tags( $post->post_excerpt );
	$output = str_replace( '"', '\'', $output );
	if ( post_password_required($post) ) {
		$output = __( 'There is no excerpt because this is a protected post.', 'prism');
		return $output;
	}
...
/**
 * prism_excerpt_rss function.
 *
 * @return $output
 */
function prism_excerpt_rss() {
	global $post;
	$output = '';
	$output = strip_tags( $post->post_excerpt );
	if ( post_password_required( $post ) ) {
	...
	return apply_filters( 'prism_excerpt_rss', $output );

}

add_filter( 'prism_excerpt_rss', 'prism_trim_excerpt' );


/**
 * Create nice multi_tag_title
 */
function prism_tag_query() {
 ...
 *
 * @todo deprcate when prism_body_class becomes a filter of body_class
 * @return $term->name
 */
function prism_get_term_name() {
 ...
 * @return bool
 */
function prism_is_custom_post_type() {
	global $post; 
