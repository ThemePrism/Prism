...
if ( function_exists( 'childtheme_override_body_class' ) )  {
	 ...
	 */
	function prism_body_class( $c ) {
		_deprecated_function( 'prism_body_class', '1.0.1.3', 'filter body_class()' );
		global $wp_query, $current_user, $blog_id, $post, $taxonomy;
	    
	    $c = array();
	
		if ( apply_filters('prism_show_bc_wordpress', TRUE ) ) {
	        // It's surely a WordPress blog, right?
	        $c[] = 'wordpress';
	    }
	    
	    if ( apply_filters( 'prism_show_bc_blogid', TRUE) ) {
	    	// Applies the blog id to BODY element .. blog-id1 for WordPress < 3.0
	    	$c[] = 'blogid-' . $blog_id;
	    }
	
		if ( apply_filters( 'prism_show_bc_datetime', TRUE) ) {
	        // Applies the time- and date-based classes (below) to BODY element
	        prism_date_classes( time(), $c );
	    }
	
	    if ( apply_filters( 'prism_show_bc_contenttype', TRUE ) ) {
	    ...
	    if ( apply_filters( 'prism_show_bc_singular', TRUE) ) {
	...
	if ( is_single() && apply_filters( 'prism_show_bc_singlepost', TRUE ) ) {
			...
			if ( isset( $wp_query->post->post_date ) )
				prism_date_classes( mysql2date( 'U', $wp_query->post->post_date ), $c, 's-' );
		...
		elseif ( is_author() && apply_filters( 'prism_show_bc_authorarchives', TRUE ) ) {
		...
		elseif ( is_category() && apply_filters( 'prism_show_bc_categoryarchives', TRUE ) ) {
		...
		elseif ( is_tag() && apply_filters('prism_show_bc_tagarchives', TRUE ) ) {
		...
		elseif ( is_tax() && apply_filters( 'prism_show_bc_taxonomyarchives', TRUE) ) {
			$c[] = 'taxonomy';
			$c[] = 'tax-' . $taxonomy;
			$c[] = $taxonomy . '-' . strtolower(prism_get_term_name());
		}
	
		// Page author for BODY on 'pages'
		elseif ( is_page() && apply_filters( 'prism_show_bc_pages', TRUE ) ) {
		...
		elseif ( is_search() && apply_filters( 'prism_show_bc_search', TRUE ) ) {
			the_post();
		...
		if ( apply_filters( 'prism_show_bc_loggedin', TRUE ) ) {
	 ...
	 // Paged classes; for page x > 1 classes of index and all post types etc.
		if ( isset( $post ) && apply_filters( 'prism_show_bc_pagex', TRUE ) ) {
		...
		// And tada!
		return array_unique(apply_filters( 'prism_body_class', $c )); // Available filter: prism_body_class
	}
}

// Add Legacy Body Classes to body_class()
if ( current_theme_supports ( 'prism_legacy_body_class' ) ) {
	add_filter( 'body_class', 'prism_body_class', 20 );
}

// Add browser CSS class to the end (queuing through priority) of the body classes 
if ( apply_filters( 'prism_show_bc_browser', TRUE ) ) {
	add_filter( 'body_class', 'prism_browser_class_names', 30 ); 
}

/**
 * prism_browser_class_names function.
 */
function prism_browser_class_names($classes) {
